...
import net.minecraft.util.ObjectIntIdentityMap; // import net.minecraft.util.collection.IdList;
import net.minecraft.util.palette.IPalette; // import net.minecraft.world.chunk.Palette;
import net.minecraft.util.palette.IResizeCallback; // import net.minecraft.world.chunk.PaletteResizeListener;
 ...
 * {@link ga.ozli.minecraftmods.swift.common.lithium.world.chunk.LithiumHashPalette#idFor(Object)} through using a faster backing map and reducing pointer chasing.
 */
public class LithiumHashPalette<T> implements IPalette<T> {
    ...
    public void write(PacketBuffer buf) {
        ...
        for (int i = 0; i < size; ++i) {
            buf.writeVarInt((Integer) this.idList.getByValue((Integer) this.get(i)));
        }
    }
    ...
    public int getSerializedSize() {
        ...
        for (int i = 0; i < this.size; ++i) {
            /* todo: remove the need for casting on Forge, see Fabric's Int2ObjectBiMap and Forge's IntIdentityHashBiMap
             *       will probably need to have a mixin for `public K getByValue(int value)` so that it doesn't cast to K
             *
             * scratch that, didn't work. I'm not sure what the fix is then :/
             */
            size += PacketBuffer.getVarIntSize((Integer) this.idList.getByValue((Integer) this.get(i)));
        }
